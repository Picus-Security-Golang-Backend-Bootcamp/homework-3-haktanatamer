...
const (
	...
	isAdd          bool   = false //true yaparsak kitap ve yazar ekleme özelliği açılıyor
)
...
//fillDb() veriler csv okunup db basılıyor
func fillDb() {

	bookCsv, _ := helper.ReadBookCsv("book.csv")
	bookList = book.InitializeBookList(bookCsv)

	bookRepository.InsertInitialData(bookList.BookList)

	authorCsv, _ := helper.ReadAuthorCsv("author.csv")
	authorList = author.InitializeAuthorList(authorCsv)

	authorRepository.InsertInitialData(authorList.AuthorList)
}
